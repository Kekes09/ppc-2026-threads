...
bool LuchnikovEMultOfDenseMatrixFoxAlgoritmSTL::RunImpl() {
  ...
  if (n % block_size_ != 0 || num_blks == 0) {
    ...
    for (int stage = 0; stage < num_blks; ++stage) {
      FoxStageWorker worker{matrix_a_.data(), matrix_b_.data(), matrix_c_.data(), n, block_size_, stage, num_blks};

      std::for_each(std::execution::par, block_indices.begin(), block_indices.end(), worker);
    }
  }
  return true;
}
