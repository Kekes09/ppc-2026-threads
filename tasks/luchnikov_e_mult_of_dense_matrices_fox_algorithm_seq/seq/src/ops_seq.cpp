...
bool LoadMatrixFromFile(const std::string& file_path, DenseMatrix& mat) {
    ...
    if (input.fail() || mat.rows <= 0 || mat.cols <= 0) return false;
...
void MultiplyBlocks(const DenseMatrix& a, const DenseMatrix& b, DenseMatrix& res,
...
void ExecuteFoxAlgorithm(const DenseMatrix& a, const DenseMatrix& b, DenseMatrix& res, int blk) {
    ...
    for (int s = 0; s < stages; ++s) {
        for (int i = 0; i < stages; ++i) {
            int broadcast_idx = (i + s) % stages;
            for (int j = 0; j < stages; ++j) {
                MultiplyBlocks(a, b, res,
                               i * blk, j * blk, blk,
...
bool LuchnikovEMultOfDenseMatrixFoxAlgoritmSeq::PreProcessingImpl() {
    if (!LoadMatrixFromFile(path_a_, matrix_a_)) return false;
    if (!LoadMatrixFromFile(path_b_, matrix_b_)) return false;
